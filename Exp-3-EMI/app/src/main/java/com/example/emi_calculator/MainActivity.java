...
public class MainActivity extends AppCompatActivity {
    ...
    public void calculateEMI(View view){
        ...
        firstPart = mortgage * monthlyIR;
        numerator = Math.pow(1 + monthlyIR, months);
        denominator = (Math.pow(1 + monthlyIR, months) - 1);
        fraction = numerator / denominator;
        emi = firstPart * fraction;

        //output EMI to outputLabel
        TextView outputLabel = (TextView) findViewById(R.id.outputLabel);
        DecimalFormat rupees = new DecimalFormat("0.00");//format rupees
